...
function  help(inputFilePath: string) {
    console.log(`Help:`);
...
async function  main() {  // Application
    ...
    if (process.argv.length <= 2) {
        ...
        var  inputLines: string[] = await readStdInAll();
    } else {
        const  inputFilePath = process.argv[2];
        if (fs.existsSync(inputFilePath)) {
            var  inputLines: string[] = readFile(inputFilePath);
        } else {
            help(inputFilePath);
            return;
        }
...
var  MainPID = "";
var  CurrentPID = "";
var  CurrentBreadcrumbsIn: {[pID: string]: string} = {};
var  LogIDs: number[] = [];
var  LabelIdIn: {[pID: string]: number} = {};
...
function  getLeftOfFirstKeyword(target: string, keyword: string): string {
...
function  _<T>(value: T): ElseIfMethodChain<TOrReturnType<T>> {
